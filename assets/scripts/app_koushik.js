...
function reset() {
  ...
  if (!hasBonusLife) {
    hasBonusLife = true; // changes for bonus life reset in UI

    bonusLifeEl.innerText = '1';
  }
...
function endRound() {
  ...
  if (currentMonsterHealth <= 0 && currentPlayerHealth > 0) {
    ...
    alert('Match Draw!');
  } // if ( //   (currentMonsterHealth <= 0 && currentPlayerHealth >= 0) || //   (currentMonsterHealth <= 0 && currentMonsterHealth >= 0) || //   (currentMonsterHealth <= 0 && currentPlayerHealth <= 0) // ) { //   reset(); // }
